...
import { getWorkoutImagesOneThunk } from "../../redux/workoutImages";
...
function WorkoutPageForm() {
  ...
  const workoutImageObject = useSelector((state) => state.workoutimages?.single);

  useEffect(() => {
    const fetchWorkoutImages = async () => {
      if (workoutObj?.id) {
        await dispatch(getWorkoutImagesOneThunk(workoutObj.id));
      }
    };
    fetchWorkoutImages();
  }, [dispatch, workoutObj]);

  // Separate useEffect to log workoutImageObject once updated
  useEffect(() => {
    if (workoutImageObject?.id) {
      console.log("workoutImageObject.url = ", workoutImageObject.url);
    }
  }, [workoutImageObject]);
  ...
  return (
    <div className="mainBodyStyle">
      ...
      {workoutImageObject?.url && (
        <div className="workout-image">
          <img src={workoutImageObject.url} alt="Workout" style={{ maxWidth: '100%', height: 'auto' }} />
        </div>
      )}
    </div>
  );
